...
// Current locale or default to 'tr'
const currentLocale = computed(() => locale.value || 'tr')

// Collection name based on locale
const collectionName = computed(() => `blog_${currentLocale.value}`)

// Extract slug from route (remove /blog/ prefix and locale prefix)
const slug = computed(() => {
  let path = route.path
  ...
  // Remove /blog/ prefix to get just the slug
  return path.replace('/blog/', '')
})

// Fetch the article from collection by path
const { data: article } = await useAsyncData(`article-${route.path}`, async () => {
  try {
    // Query the collection for article matching the slug
    const result = await queryCollection(collectionName.value as any)
      .path(`/${currentLocale.value}/blog/${slug.value}`)
      .first()
    return result
  } catch (e) {
